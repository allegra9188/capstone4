...
function Insights() {
  ...
  const renderItem = (item, index) => (
    <div className="lobbying-card" key={index}>
      ...
      <p><span id="bold">Specific Issues: </span>{item.specific_issues_0}</p>
      <p><span id="bold">Lobbying Client: </span>{item.lobbying_client_name}</p>
      <p>Lobbying Client Description: {item.lobbying_client_general_description}</p>
      <p>Lobbying Registrant: {item.lobbying_registrant_name}</p>
      <p>Lobbying Registrant Description: {item.lobbying_registrant_general_description}</p>
      ...
      <p>Latest Filing URL: <a href={item.latest_filing_pdf_url}>{item.latest_filing_pdf_url}</a></p>
    </div>
  );
